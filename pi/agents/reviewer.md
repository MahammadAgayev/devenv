...
3. Report each issue found (see Output below).
...
<output>
For each finding, report as markdown:

### `file_path:line_start-line_end` — Title (P0-P3)
One-paragraph body: bug, trigger, impact. Include a `suggestion` code block with concrete replacement code when applicable.

After all findings, close with:
