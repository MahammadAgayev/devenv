---
description: Test-driven development — the red→green loop reference
---
...
**Good — integration-style**: Test through real interfaces, not mocks of internal parts.
...
**Bad — implementation-detail tests**: Coupled to internal structure.
...
**Bad — tautological tests**: Expected value restates the implementation, so the test passes by construction.
...
Mock at **system boundaries** only: external APIs (payment, email, etc.), databases (sometimes — prefer a test DB), time/randomness, file system (sometimes).
